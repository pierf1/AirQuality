...
class AirqualityPlugin(octoprint.plugin.SettingsPlugin,
	...
	def get_template_configs(self):
		templates_to_load = [{'type': "navbar", 'custom_bindings': False},
                             {'type': "settings", 'custom_bindings': True},
                             {'type': "tab", 'custom_bindings': True, 'template': "airquality_tab.jinja2"}]
	...
	def on_api_command(self, command, data):
		...
		elif command == "get_airquality_data":
			db = sqlite3.connect(self.db_path)
			cursor = db.cursor()
			cursor.execute(
				'''SELECT inserted_timestamp,  raw1, raw2_5, raw10, pm1, pm2_5, pm10''')
			response = {'aire_quality': cursor.fetchall()}
			db.close()
			self._logger.error("Error getting database information for graphing")
