...
$(function() {
    function AirQualityViewModel(parameters) {
        ...
        self.onTabChange = function(current, previous) {
            if (current === "#tab_plugin_airquality"){
                self.plotAirqualityData(false);
                console.log("Not on AirQuality Tab");
            }
    };

        self.plotAirqualityData = function(data) {
            $.ajax({
                url: API_BASEURL + "plugin/airquality",
                type: "POST",
                dataType: "json",
                data: JSON.stringify({
                    command: "get_airquality_data"
                }),
                contentType: "application/json; charset=UTF-8"
            }).done(function(data){
                    var trace_raw1 = {x:[],y:[],mode:'lines+markers',name: 'raw1'}
                    var trace_raw2_5 = {x:[],y:[],mode:'lines+markers',name: 'raw2.5'}
                    var trace_raw10 = {x:[],y:[],mode:'lines+markers',name: 'raw10'}
                    var trace_pm1 = {x:[],y:[],mode:'lines+markers',name: 'pm1'}
                    var trace_pm2_5 = {x:[],y:[],mode:'lines+markers',name: 'pm2.5'}
                    var trace_pm10 = {x:[],y:[],mode:'lines+markers',name: 'pm10'}

                    ko.utils.arrayForEach(data.energy_data, function(row){
                        trace_raw1.x.push(row[0]);
                        trace_raw1.y.push(row[1]);
                        trace_raw2_5.x.push(row[0]);
                        trace_raw2_5.y.push(row[2]);
                        trace_raw10.x.push(row[0]);
                        trace_raw10.y.push(row[3]);
                        trace_pm1.x.push(row[0]);
                        trace_pm1.y.push(row[4]);
                        trace_pm2_5.x.push(row[0]);
                        trace_pm2_5.y.push(row[5]);
                        trace_pm10.x.push(row[0]);
                        trace_pm10.y.push(row[6]);
                    });
                    var layout = {title:'Air Quality Data',
    ...
    OCTOPRINT_VIEWMODELS.push({
        ...
        dependencies: ["settingsViewModel","loginStateViewModel","filesViewModel"],
        elements: ["#settings_plugin_airquality","#tab_plugin_airquality"]
    });
});
